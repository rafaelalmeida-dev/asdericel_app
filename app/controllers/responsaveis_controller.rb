class ResponsaveisController < ApplicationController
  ...
  add_breadcrumb "Responsaveis", :responsaveis_path

  def index
    @q = Responsavel.ransack(params[:q])
    @pagy, @responsaveis = pagy(@q.result)
  end
  ...
  def create
    ...
    if @responsavel.save
      redirect_to responsaveis_path, notice: t("messages.created_successfully")
    else
  ...
  def update
    if @responsavel.update(responsavel_params)
      redirect_to responsaveis_path, notice: t("messages.updated_successfully"), status: :see_other
    else
  ...
  def destroy
    if @responsavel.destroy
      redirect_to responsaveis_url, notice: t("messages.deleted_successfully")
    else
      redirect_to responsaveis_url, alert: t("messages.delete_failed_due_to_dependencies")
    end
  end
  ...
  def set_responsavel
    ...
    redirect_to responsaveis_path, alert: t("messages.not_found") unless @responsavel
  end
